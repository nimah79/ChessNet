...
public class RadixTree<V extends Serializable>
	...
	private static final long serialVersionUID = -376377459533925802L;
	...
	/**
	 * Traverses this radix tree using the given visitor. Only values with the
	 * given prefix will be visited. Note that the tree will be traversed in
	 * lexicographical order.
	 * 
	 * @param visitor
	 *            the visitor
	 * @param prefix
	 *            the prefix used to restrict visitation
	 */
	 ...
	 *            the visitor
	 */
	private void visit(RadixTreeNode<V> node, String prefixAllowed,
			String prefix, RadixTreeVisitor<V, ?> visitor) {
		...
		for (RadixTreeNode<V> child : node) {
			...
			if (prefixAllowed.length() <= prefixLen
					...
					|| newPrefix.charAt(prefixLen) == prefixAllowed
							.charAt(prefixLen)) {
	...
	/**
	 * Remove the value with the given key from the subtree rooted at the given
	 * node.
	 * 
	 * @param key
	 *            the key
	...
	private V put(String key, V value, RadixTreeNode<V> node) {
		V ret = null;
		final int largestPrefix = RadixTreeUtil.largestPrefixLength(key,
				node.getPrefix());
		if (largestPrefix == node.getPrefix().length()
			...
			// Key is bigger than the prefix located at this node, so we need to
			// see if
			// there's a child that can possibly share a prefix, and if not, we
			// just add
			...
			if (!found) {
				// No child exists with any prefix of the given key, so add a
				// new one
			...
			final String leftoverPrefix = node.getPrefix()
					.substring(largestPrefix);
			final RadixTreeNode<V> n = new RadixTreeNode<V>(leftoverPrefix,
					node.getValue());
			...
			if (largestPrefix == key.length()) {
				// The largest prefix is equal to the key, so set this node's
				// value
				ret = node.getValue();
				node.setValue(value);
				node.setHasValue(true);
				...
				final String leftoverKey = key.substring(largestPrefix);
				final RadixTreeNode<V> keyNode = new RadixTreeNode<V>(
						leftoverKey, value);
	 ...
	 *         there was no mapping for <code>key</code>
	 */
			...
			if (largestPrefix == child.getPrefix().length()
				...
				if (child.getChildren().isEmpty()) {
					...
					if (child.getChildren().size() == 1) {
						// The subchild's prefix can be reused, with a little
						// modification
						final RadixTreeNode<V> subchild = child.getChildren()
								.iterator().next();
						final String newPrefix = child.getPrefix()
								+ subchild.getPrefix();
